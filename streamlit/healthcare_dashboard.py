...
st.title("Healthcare RxDecision Analytics Platform")
...
with tab6:
    ...
    loc_df = session.sql("SELECT REGION, COUNT(*) AS LOCATIONS FROM RAW_DB.RAW_SCHEMA.HCO_LOCATIONS WHERE REGION IS NOT NULL GROUP BY REGION ORDER BY LOCATIONS DESC").to_pandas()
    if not loc_df.empty:
        st.altair_chart(alt.Chart(loc_df).mark_bar(color="#00CEC9").encode(x="LOCATIONS:Q", y=alt.Y("REGION:N", sort="-x")).properties(height=200), use_container_width=True)
...
st.divider()
st.caption("Healthcare RxDecision Analytics Platform | Data: Snowflake Marketplace (Definitive Healthcare) | Account: tyb42779 | Author: DEVIKAPG")
